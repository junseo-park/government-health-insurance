...
def clean(valuename, filename):
    df = pd.read_csv('data/' + filename + '/' + filename + '.csv', header=2)
    df = df.drop(['Indicator Name', 'Indicator Code', '2016'], axis=1)
    df = delete_rows(df)
    df2 = pd.melt(df, id_vars=['Country Name', 'Country Code'], var_name='Year', value_name=valuename)
    df2.replace(to_replace='Korea, Dem. People’s Rep.', value='Korea, Dem. People\'s Rep.', inplace=True)
    df2.to_csv('0_preprocessing/' + filename + '.csv', sep=';')

clean('Life Expectancy', 'lifeexpectancy')
clean('Infant Mortality', 'infantmortality')
clean('GDP', 'gdp')
clean('Net Aid', 'netaid')
clean('Health Expenditure', 'healthexpenditure')
clean('Military Expenditure', 'militaryexpenditure')
clean('Education Expenditure', 'educationexpenditure')
clean('Research Expenditure', 'researchexpenditure')